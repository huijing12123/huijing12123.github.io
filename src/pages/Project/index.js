...
function Project() {
    ...
    const { size, ...rest } = useSpring({
        ref: springApi,
        config: config.stiff,
        from: { size: '60%', background: '#eebbc3', boxShadow: '8px 8px 10px rgba(0, 0, 0, 0.2)', display: "flex", justifyContent: "center", alignItems: "center" },
        to: {
    ...
    const { size: sizeInternship, ...restInternship } = useSpring({
        ...
        config: config.stiff,
        from: { size: '60%', background: '#b8c1ec', boxShadow: '8px 8px 10px rgba(0, 0, 0, 0.2)', display: "flex", justifyContent: "center", alignItems: "center" },
        to: {
    ...
    useChain(openInternship ? [springApiInternship, transApiInternship] : [transApiInternship, springApiInternship], [
        0,
        openInternship ? 0.1 : 0.6,
    ]);
    ...
    return (
        <div className={styles.wrapper}>
            <AnimatedContainer
                rest={rest}
                size={size}
                onClick={() => setOpenGrosaver(open => !open)}
            >
